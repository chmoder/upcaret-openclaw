...
- Default cap: **4 total `web_fetch` calls**. Expand to **6** only when a strong event/resources hub clearly links to speaker details.
...
3. If an event source is published as a document/media URL (PDF/DOCX/PPTX/XLSX/ZIP/EPUB/image/audio), run the MarkItDown MCP tool (`convert_to_markdown(uri)`) and validate speaker evidence from the converted Markdown.
