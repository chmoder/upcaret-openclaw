...
- Follow discovered links only when they are high-confidence candidates for event/speaking evidence.
- Keep a visited-URL set and skip duplicates.
- Default cap: **4 total `browser navigate` calls**. Expand to **6** only when a strong event/resources hub clearly links to speaker details.
- Depth 1 by default; depth 2 only for clear event-listing -> event-detail transitions.
- Skip low-value targets (`.jpg`, `.png`, `.gif`, `.zip`, trackers, login/cart pages). Documents are allowed when likely to contain agenda/speaker listings.
- Stop early once you have verified speaking engagements.
...
2. Use `browser navigate` to open high-confidence pages, then `browser snapshot` to read the full rendered content. Follow only high-confidence candidate links such as:
   - `events`, `webinars`, `speakers`, `conference`, `agenda`, `media`
   - event detail pages that list speakers by name
   - advisor profile pages with speaking sections

3. If an event source is published as a document/media URL (PDF/DOCX/PPTX/XLSX/ZIP/EPUB/image/audio), run the MarkItDown MCP tool (`convert_to_markdown(uri)`) instead of `browser navigate` and validate speaker evidence from the converted Markdown.

4. Only include events with a verifiable name, date, or topic where the advisor is individually listed as a speaker.
