...
- Default cap: **4 total `web_fetch` calls**. Expand to **6** only when a strong team/staff directory clearly links to person detail pages.
...
3. Extract links from fetched pages and follow only high-confidence candidate links such as:
...
4. If a fetched or discovered URL is a document/media file (PDF/DOCX/PPTX/XLSX/ZIP/EPUB/image/audio), use the MarkItDown MCP tool (`convert_to_markdown(uri)`) instead of `web_fetch` and extract colleague names from the returned Markdown (e.g. firm brochures, team rosters, org charts).
